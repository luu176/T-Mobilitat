...
class PTGui(tk.Tk):
    def __init__(self, data: dict):
        ...
        self.detail_scroll = ttk.Scrollbar(self.detail_outer, orient="vertical", command=self.detail_canvas.yview)
    ...
    def _big_button(self, parent, text, command):
        b = tk.Button(parent, text=text, command=command, font=LARGE_FONT, width=18, height=2)
        return b
    ...
    def _build_right(self):
        ...
        ttk.Button(self.right, text="Copy current view", command=self.copy_summary).pack(fill="x", pady=4)
        ttk.Button(self.right, text="Export current view", command=self.export_shown).pack(fill="x", pady=4)
        ttk.Button(self.right, text="Open raw JSON in window", command=self._open_raw_popup).pack(fill="x", pady=4)
    ...
    def _build_bottom(self):
        ...
        tk.Button(left_actions, text="Copy summary to clipboard", command=self.copy_summary).pack(side=tk.LEFT, padx=6)
        tk.Button(left_actions, text="Export shown text", command=self.export_shown).pack(side=tk.LEFT, padx=6)

        tk.Button(right_actions, text="Quit", command=self.quit).pack(side=tk.RIGHT, padx=6)
    ...
    def _label_pair(self, parent, key, val, key_font=None, val_font=None, wraplength=700):
        if key_font is None:
            key_font = LARGE_FONT
        if val_font is None:
            val_font = LARGE_FONT
        ...
        tk.Label(f, text=f"{val}", font=val_font, wraplength=wraplength, justify="left").pack(side=tk.LEFT, anchor="w", padx=8)
        return f
    ...
    def _open_raw_popup(self):
        ...
        txt = scrolledtext.ScrolledText(popup, wrap=tk.WORD, font=MONO, width=90, height=35)
    ...
    def show_summary(self):
        ...
        # Big card preview
        card_frame = tk.Frame(self.detail_frame, relief="raised", borderwidth=2, padx=12, pady=12)
        ...
        lbl_num = tk.Label(color_area, text=str(sus.get('number', '—')), font=BIG_NUM_FONT, bg=bg)
        lbl_num.pack(anchor='center')

        meta = tk.Frame(color_area, bg=bg)
        meta.pack(anchor='center', pady=8)
        tk.Label(meta, text=f"Issuer: {sus.get('issuer_name', '—')}", font=LARGE_FONT, bg=bg).pack()
        tk.Label(meta, text=f"Template: {sus.get('template_name', '—')}", font=LARGE_FONT, bg=bg).pack()
        ...
        if active_pass:
            ap = tk.Frame(self.detail_frame, relief='groove', borderwidth=1, padx=8, pady=8)
            ap.pack(fill='x', pady=8)
            tk.Label(ap, text="Active pass", font=XL_FONT).pack(anchor='w')
            tk.Label(ap, text=f"{active_pass.get('name')} — owner: {active_pass.get('owner_name')}", font=LARGE_FONT).pack(anchor='w')
            tk.Label(ap, text=f"Zones: {active_pass.get('zones')}  |  Trip balance: {active_pass.get('trip_balance')}", font=LARGE_FONT).pack(anchor='w')
        ...
        if active_pass:
            summary_text.append(f"Active pass: {active_pass.get('name')} (zones {active_pass.get('zones')})")
        self.current_export_text = "\n".join(summary_text)

    def show_sus(self):
        self._clear_detail()
        sus = self.data.get('sus', {})
        tk.Label(self.detail_frame, text="SUS (Card) Details", font=XL_FONT).pack(anchor='w')
    ...
    def show_atiu(self):
        ...
        tk.Label(self.detail_frame, text="ATIU (App) Details", font=XL_FONT).pack(anchor='w')
    ...
    def show_user(self):
        ...
        tk.Label(self.detail_frame, text="User / Personalisation", font=XL_FONT).pack(anchor='w')
        self._label_pair(self.detail_frame, 'Identification status:', user.get('identification_status_text'))
        self._label_pair(self.detail_frame, 'Language:', user.get('language_desc'))
        self._label_pair(self.detail_frame, 'Sensory aids:', user.get('sensory_aids_desc'))
        self.current_export_text = pretty_json(user)

    def show_passes(self):
        self._clear_detail()
        passes = self.data.get('passes', {})
        tk.Label(self.detail_frame, text="Passes & Loads", font=XL_FONT).pack(anchor='w')

        selected = passes.get('selected_pass')
        tk.Label(self.detail_frame, text=f"Selected pass index: {selected}", font=LARGE_FONT).pack(anchor='w')

        instances = passes.get('instances', [])
        if not instances:
            tk.Label(self.detail_frame, text="No pass instances found.", font=LARGE_FONT).pack(anchor='w')
            return

        for inst in instances:
            frame = tk.Frame(self.detail_frame, relief='ridge', borderwidth=1, padx=8, pady=8)
            frame.pack(fill='x', pady=6)
            title = f"{inst.get('name')}" + ("  (ACTIVE)" if inst.get('is_the_active_pass') else "")
            tk.Label(frame, text=title, font=LARGE_FONT).pack(anchor='w')
            tk.Label(frame, text=f"Owner: {inst.get('owner_name')}  |  Zones: {inst.get('zones')}", font=LARGE_FONT).pack(anchor='w')
            tk.Label(frame, text=f"Trip balance: {inst.get('trip_balance')}  |  Purse balance: {inst.get('purse_balance')}", font=LARGE_FONT).pack(anchor='w')
            ...
            if loads:
                ...
                for l in loads:
                    lf = tk.Frame(loads_frame, relief='groove', borderwidth=1, padx=6, pady=6)
                    ...
                    tk.Label(lf, text=f"Trip balance: {l.get('trip_balance')}", font=SMALL).pack()
                    tk.Label(lf, text=f"Sale: {l.get('sale_datetime')}", font=SMALL).pack()
                    # view details button
                    tk.Button(lf, text='View', command=lambda d=l: self._open_small_json(d)).pack(pady=4)
    ...
    def show_last_trip(self):
        ...
        self._label_pair(self.detail_frame, 'First stage datetime:', lt.get('first_stage_datetime'))
        self._label_pair(self.detail_frame, 'Trip spent units:', lt.get('trip_spent_units'))
        self._label_pair(self.detail_frame, 'Num transfers:', lt.get('num_transfers'))

        stages = lt.get('stages', [])
        if stages:
            tk.Label(self.detail_frame, text='Stages:', font=LARGE_FONT).pack(anchor='w', pady=(6,0))
            for s in stages:
                sf = tk.Frame(self.detail_frame, padx=6, pady=6, relief='groove', borderwidth=1)
                sf.pack(fill='x', pady=4)
                tk.Label(sf, text=f"Stage {s.get('index')} — {s.get('company_name')}", font=LARGE_FONT).pack(anchor='w')
                on = s.get('on_station', {})
                entry = on.get('entry')
                if entry:
                    tk.Label(sf, text=f"Entry station: {entry.get('station_interop_name')} @ {entry.get('datetime')}", font=LARGE_FONT).pack(anchor='w')
                exit = on.get('exit')
                if exit and exit.get('station_interop_name'):
                    tk.Label(sf, text=f"Exit station: {exit.get('station_interop_name')}", font=LARGE_FONT).pack(anchor='w')
    ...
    def show_sus_image(self):
        ...
        tk.Label(self.detail_frame, text='SUS Image (hex)', font=XL_FONT).pack(anchor='w')
        if not data:
            tk.Label(self.detail_frame, text='No sus_image data present.', font=LARGE_FONT).pack(anchor='w')
            return
        # present in a monospaced scrolled text
        txt = scrolledtext.ScrolledText(self.detail_frame, wrap=tk.NONE, font=MONO, height=12)
    ...
    def show_raw(self):
        ...
        txt = scrolledtext.ScrolledText(self.detail_frame, wrap=tk.WORD, font=MONO, width=100, height=30)
        txt.pack(fill='both', expand=True)
    ...
    def _open_small_json(self, obj):
        ...
        txt = scrolledtext.ScrolledText(popup, wrap=tk.WORD, font=MONO, width=60, height=20)
        txt.pack(fill='both', expand=True)
